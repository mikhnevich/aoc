...
object D12 {
  ...
  def splitIntoObjects(s: String): List[Value] = {
    ...
    var found = false
    var i = 0
    while (t.nonEmpty && i < t.length) {
      if (t(i) == '[') b1 = b1 + 1
      else if (t(i) == '{') b2 = b2 + 1
      else if (t(i) == ']') b1 = b1 - 1
      else if (t(i) == '}') b2 = b2 - 1
      else if (t(i) == ':') colon = true
      else if (t(i) == ',' && b1 == 0 && b2 == 0) {
        val x = t.substring(0, i)
        if (x(0) == '[') {
          result = result ++ List(new ArrayValue(splitIntoObjects(x.drop(1).init)))
        } else if (x(0) == '{') {
          result = result ++ List(new ObjectValue(splitIntoObjects(x.drop(1).init)))
        } else if (colon) {
          result = result ++ List(parseProperty(x))
        } else {
          result = result ++ List(new SingleValue(x))
        }
        t = t.drop(i + 1)
        i = -1
        colon = false
    ...
    if (t.nonEmpty) {
      if (b1 != 0) throw new IllegalArgumentException(s"Unmatched string: $t")
      else {
        if (t(0) == '[') {
          result = result ++ List(new ArrayValue(splitIntoObjects(t.drop(1).init)))
        } else if (t(0) == '{') {
          result = result ++ List(new ObjectValue(splitIntoObjects(t.drop(1).init)))
        } else if (colon) {
          result = result ++ List(parseProperty(t))
        } else {
          result = result ++ List(new SingleValue(t))
        }
      }
    }
    result
  ...
  def parse(s: String) = {
    val objects = splitIntoObjects(s)
  }

  def sum(values: List[Value]): Int = {
    if (values.isEmpty) 0
    else {
      val s = values.head match {
        case v: SingleValue => intValue(v.s)
        case v: PropertyValue => sum(List(v.value))
        case v: ArrayValue => sum(v.children)
        case v: ObjectValue => sum(v.children)
  ...
  def hasRedProperty(v: ObjectValue): Boolean = {
    v.children.exists {
      case value: PropertyValue => value.value match {
        case s: SingleValue => s.s == "red"
        case _ => false
      }
      case _ => false
    }
  ...
  def sumSkipRed(values: List[Value]): Int = {
    if (values.isEmpty) 0
    else {
      val s = values.head match {
        case v: SingleValue => intValue(v.s)
        case v: PropertyValue => sumSkipRed(List(v.value))
        case v: ArrayValue => sumSkipRed(v.children)
        case v: ObjectValue => if (hasRedProperty(v)) 0 else sumSkipRed(v.children)
        case _ => throw new IllegalStateException("")
      }
      s + sumSkipRed(values.tail)
    }
  }

  def intValue(s: String): Int = {
    try {
      var t = s.trim
      if (t(0) == '"') t = t.tail.init
      t.toInt
    } catch {
      case e: Exception => 0
    }
  ...
  def main(args: Array[String]): Unit = {
    val test = """{"z":8,"a":["red",{"c":"2","b":2},3],"c":12}"""
    val test2 = """[1,{"c":"red","b":2},3]"""
    val a = splitIntoObjects(input)
    println(a.mkString("\n"))
    println(sum(a))
    println(sumSkipRed(a))
  }


  def part1: Unit = {
    val r = input.replaceAll("\\{|\\}|\\,|\\:|\\[|\\]|\"", " ")
    val parts = r.split(" ")
    var sum = 0
    for (p <- parts) {
      try {
        sum = sum + p.trim.toInt
      } catch {
        case e: Exception =>
      }
    }
